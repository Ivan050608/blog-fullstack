...
//         // "https://blogapi-o0fk.onrender.com/posts/getPosts",
//         "http://localhost:4000/posts/getPosts",
//          {
//         method: "GET",
//         headers: {
...
//     try {
//       const response = await fetch(
//         // `https://blogapi-o0fk.onrender.com/posts/deletePost/${postId}`, {
    ...
    <>
      <Card className="p-4 shadow-sm mb-4">
        <Card.Body>
          <Card.Title className="text-center text-2xl font-bold">Blog Posts</Card.Title>
          <div className="text-center mb-3">
            <Button variant="dark" onClick={() => setShowAddModal(true)}>Add Post</Button>
          </div>
          <Row>
            {posts.length > 0 ? (
              posts.map((post) => (
                <BlogCard key={post._id} post={post} handleEdit={handleEditPost} handleDelete={handleDeletePost} />
              ))
            ) : (
              <p className="text-center text-gray-500">No posts available.</p>
            )}
